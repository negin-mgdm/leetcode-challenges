...
var largestInteger = function (num) {
    ...
    for (let digit of number) {
        if (parseInt(digit) % 2 == 0) {
            evenNums.push(parseInt(digit));
        } else {
            oddNums.push(parseInt(digit));
        }
    }
    ...
    for (let digit of number) {
        if (parseInt(digit) % 2 == 0) {
            result += evenNums[0].toString();
            evenNums.shift().toString();
        } else {
            result += oddNums[0].toString();
            oddNums.shift().toString();
        }
    }
